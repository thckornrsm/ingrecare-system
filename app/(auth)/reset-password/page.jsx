...
function parseAuthFromUrl(searchParams) {
  // 1) Supabase ส่งเป็น ?code=...
  const code = searchParams.get('code');
  if (code) return { kind: 'supabase-code', code };

  // 2) Supabase ส่งเป็น hash: #access_token=...&refresh_token=...&type=recovery
  if (typeof window !== 'undefined' && window.location.hash) {
    ...
    if (access_token && refresh_token) {
      return { kind: 'supabase-hash', access_token, refresh_token, type };
    }
  }

  // 3) โฟลว์ custom เดิมของคุณ: ?token=...
  const token = searchParams.get('token');
  if (token) return { kind: 'custom-token', token };
...
const ResetPasswordForm = () => {
  ...
  const [authInfo, setAuthInfo] = useState({ kind: 'none' });
  ...
  useEffect(() => {
    const info = parseAuthFromUrl(searchParams);
    setAuthInfo(info);
    if (info.kind === 'none') setError('ไม่พบโทเคนสำหรับการรีเซ็ตรหัสผ่าน');
    setChecking(false);
  }, [searchParams]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (authInfo.kind === 'supabase-code') {
        // แลก code เป็น session แล้วอัปเดตรหัสผ่าน
      ...
      if (authInfo.kind === 'supabase-hash') {
        // ตั้ง session จาก access_token + refresh_token แล้วอัปเดตรหัสผ่าน
      ...
      if (authInfo.kind === 'custom-token') {
        // โฟลว์เดิมฝั่ง backend ของคุณ
        const res = await fetch('/api/auth/reset-password', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ token: authInfo.token, password }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || 'เกิดข้อผิดพลาดบางอย่าง');

        setMessage((data.message || 'รีเซ็ตรหัสผ่านสำเร็จ') + ' กำลังนำคุณไปยังหน้าเข้าสู่ระบบ...');
        setTimeout(() => router.push('/login'), 2000);
        return;
      }
  ...
  return (
    <div className="w-full md:w-160 md:h-229 md:flex-shrink-0 bg-white flex items-center justify-center p-8 sm:p-12 lg:p-16 max-md:py-28">
      <div className="w-full max-w-md">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800">ตั้งรหัสผ่านใหม่</h1>
        <p className="text-gray-600 mt-2 mb-6">กรุณากรอกรหัสผ่านใหม่ที่คุณต้องการใช้งาน</p>

        {checking ? (
          <div className="text-gray-600">กำลังตรวจสอบลิงก์รีเซ็ตรหัสผ่าน...</div>
        ) : (
