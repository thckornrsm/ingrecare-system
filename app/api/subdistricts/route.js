...
export async function GET(req) {
  try {
    const districtIdStr = req.nextUrl.searchParams.get('districtId');
    const districtId = Number(districtIdStr);
    if (!districtIdStr || Number.isNaN(districtId)) {
      return new Response(JSON.stringify({ error: 'districtId is required (number)' }), { status: 400 });
    }

    const subdistricts = await prisma.subdistrict.findMany({
      where: { districtId },
      orderBy: { name_th: 'asc' },
      select: { id: true, name_th: true, zipcode: true }  // ✅ เพิ่ม zipcode
    });

    return Response.json({ subdistricts });
  } catch (e) {
    console.error(e);
    return new Response(JSON.stringify({ error: 'Failed to fetch subdistricts' }), { status: 500 });
  }
}
